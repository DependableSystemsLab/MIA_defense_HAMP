...
parser.add_argument('--model_save_tag', type=str, default='0')
...
def get_soft_labels(train_label, num_classes, top1, uniform_non_top1):
    ...
    print( np.sum(new_soft_label[0]), np.sum(new_soft_label[1])  )
...
if(is_train_ref):
    for epoch in range(distil_epochs): 
        ...
        save_checkpoint_global(
            ...
            best_filename='protected_model_best-%s.pth.tar'%args.model_save_tag,
        )
        print('alpha %.1f epoch %d |   loss %.4f | tr acc %.4f | val acc %.4f | best val acc %.4f | best test acc %.4f'%(args.alpha, epoch,distil_tr_loss,tr_acc,val_acc,distil_best_acc,best_distil_test_acc), flush=True)



criterion=nn.CrossEntropyLoss()
best_model=TexasClassifier().to(device)
resume_best= os.path.join(new_model_checkpoint_dir, 'protected_model_best-%s.pth.tar'%args.model_save_tag)
