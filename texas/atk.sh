#!/bin/bash
size=15000 # specify the size of the training set
gpu=0
...
# The original DMP model we used for reporting the results in the paper was lost,
...
tag_list=(undefended-$size selena-$size hamp-$size dmp-$size advreg-$size dpsgd-$size ls-$size memguard-$size)
declare -i i=0
for target_model in $undefended $selena $hamp $dmp $advreg $dpsgd $ls $memguard; do
	...
	then
		...
		python attack.py --path $target_model  --train_size $size  \
		                 ...
		                 --memguard_path ./final-all-models/texas\_$size\_MIA_model.h5 --gpu $gpu
		isMemGuard=1
	fi
