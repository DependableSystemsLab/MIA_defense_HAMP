...
def train(train_data,labels,model,criterion,optimizer,epoch,use_cuda,num_batchs=999999,batch_size=32, uniform_reg=False):
    ...
    for ind in range(len_t):
        ...
        if uniform_reg==True:
            uniform_ = (torch.ones(batch_size,outputs.shape[1])).to(device)
            t_softmax=1
...
for i in range(K):
    ...
    resume_best=os.path.join( new_model_checkpoint_dir,'unprotected_model_best_sub_%s-%d.pth.tar'%(str(i),save_tag))
...
outputs_avg = np.zeros((L, n_classes))
