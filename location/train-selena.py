...
K = args.K
L = args.L
sub_model_indices_file_prefix = './location_selena_submodel_indices_%s'%str(args.train_size)
non_model_indices_file = './location_selena_non_model_indices_for_each_sample_%s.pkl'%str(args.train_size)
...
criterion=nn.CrossEntropyLoss()
best_model=LocationClassifier().to(device)
resume_best= os.path.join(org_model_checkpoint_dir, 'protected_model_best-%s.pth.tar'%args.model_save_tag)
...
print('\t===> SELENA model %s | train acc %.4f | val acc %.4f | test acc %.4f'%(resume_best, best_train, best_val, best_test), flush=True)
