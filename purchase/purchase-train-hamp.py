...
if(args.entropy_penalty):
    ...
    new_model_checkpoint_dir='./label-smoothing-models'
...
attack_tr_len=private_data_len # this is the shadow non-member dataset
attack_te_len=private_data_len  
distil_epochs=200
tr_frac=0.45 
...
def train_pub(train_data, labels, true_labels, model, optimizer, num_batchs=999999, batch_size=16, alpha=0.05):
...
if(is_train_ref):
    for epoch in range(distil_epochs): 
        ...
        distil_tr_loss = train_pub(private_data_tensor, private_label_modified_tensor, private_label_tensor, distil_model,
        ...
        print(' epoch %d |   loss %.4f | tr acc %.4f | val acc %.4f | best val acc %.4f | best test acc %.4f'%( epoch,distil_tr_loss,tr_acc,val_acc,distil_best_acc,best_distil_test_acc), flush=True)
...
resume_best=new_model_checkpoint_dir+'/protected_model_best.pth.tar'
...
if(args.entropy_penalty):
    ...
    print('\t===>  %s  Label Smoothing model: train acc %.4f val acc %.4f test acc %.4f'%(resume_best, best_train, best_val, best_test), flush=True)
