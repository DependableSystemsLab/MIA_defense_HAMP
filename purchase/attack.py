...
data_set= np.load(data_path + 'purchase.npy')
...
if not os.path.isfile(data_path + 'purchase_shuffle.pkl'):
    all_indices = np.arange(len(X))
    np.random.shuffle(all_indices)
    pickle.dump(all_indices,open(data_path + 'purchase_shuffle.pkl','wb'))
else:
    all_indices=pickle.load(open(data_path + 'purchase_shuffle.pkl','rb'))
...
def get_tpr(y_true, y_score, fpr_threshold=0.001, attack='None'):
    ...
    print( '\t\t===> %s: TPR %.2f%% @%.2f%%FPR | TNR %.2f%% @%.2f%%FNR | AUC %.4f'%( attack, highest_tpr*100, fpr_threshold*100, highest_tnr*100, fpr_threshold*100, auc_score  ) )
...
if(args.attack == 'entropy'):
    ...
    if(args.prepMemGuard):
        ...
        test_outputs1 = shadow_test_performance[0] # data used for training the attack (these are not members) 
