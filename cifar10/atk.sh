...
tag_list=(undefended-$size selena-$size hamp-$size dmp-$size advreg-$size memguard-$size)
declare -i i=0
for target_model in $undefended $selena $hamp $dmp $advreg $memguard; do
	...
	then
		...
		python attack.py --path $target_model  --train_size $size  \
		                 ...
		                 --memguard_path ./final-all-models/cifar10\_$size\_MIA_model.h5 --gpu $gpu
		isMemGuard=1
	fi
