...
parser.add_argument('--memguard_path', type=str, default=None)  
...
parser.add_argument('--folder_tag', type=str, default='memguard')
parser.add_argument('--res_folder', type=str, default='lira-memguard-fullMember')
...
print( os.path.join(lira_folder, '%s_keep.npy'%args.save_tag) )
if(os.path.exists(os.path.join(lira_folder, '%s_logit.npy'%args.save_tag))):
    print("== \t%s already existed!!! "%os.path.join(lira_folder, '%s_logit.npy'%args.save_tag))
    import sys
    sys.exit()
...
shadow_performance = _model_predictions(best_model, all_shadow_data_tensor, all_shadow_label_tensor, batch_size=32)  
...
for test_sample_id in np.arange(0,f_evaluate.shape[0]):
    if test_sample_id%100==0:
        print(args.save_tag, "test sample id: {}".format(test_sample_id), len(f_evaluate), flush=True)
